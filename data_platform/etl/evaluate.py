...
def evaluate(model_path: Path) -> dict:
    ...
    for img_path in image_paths:
        ...
        # Collect YOLO metrics
        if result.boxes is not None and len(result.boxes) > 0:
            all_precisions.append(float(result.boxes.conf.mean().cpu()))
    ...
    # Run official YOLO val for precision/recall/mAP
    val_results  = model.val(data=str(data_yaml), split="test", verbose=False)
    test_map50   = float(val_results.box.map50)
    test_map5095 = float(val_results.box.map)
    ...
    metrics = {
        ...
        "test_map50_95":  round(test_map5095, 4),
